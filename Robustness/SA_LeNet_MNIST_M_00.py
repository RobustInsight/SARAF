...
from Networks.LeNet5_Multiple import LeNet5M
...
class robust_simannealer(Annealer):
    ...
    def energy(self):
        ...
        model = LeNet5M(num_classes=10, Activation_Functions=Activation_Functions, states_=self.state).to(device)

        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=0.001)
        fname = '../save_ln_mnist/len_mnist_' + '_'.join(map(str, self.state)) + '.pth'
        ...
        write_data_in_file('results_LN_MN_00.csv', (self.state, self.atk, self.eps, rob_acc, test_acc))
...
if __name__ == '__main__':
    ...
    data_loaders_, dataset_sizes_ = get_mnist_data()
    # eps_s_ = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3]

    atk_eps_s = [("FGSM", 0.05)]
    for atk_eps_ in atk_eps_s:
        state_ = [1]*LAYERS + [0]*LAYERS*2
        '''
        state_ = [1, 4, 2, 1, 0.25, 1, 3,       0, 0, 4, 0, 0, 0, 4,      0, 0, 2, 0, 0, 0, 4]
        state_ = [2, 0.25, 2, 1, 0.25, 1, 3,    0, 1, 4, 5, 0, 0, 6,      2, 0, 2, 0, 0, 0, 4]
        state_ = [0.25, 0.25, 0.25, 1, 2, 1, 3,
         1, 1, 6, 5, 0, 0, 6,
         0, 0, 0, 0, 3, 0, 4]
        state_ = [2, 0.25, 0.25, 2, 0.25, 4, 0.25, 1, 2, 3, 4, 0, 4, 2, 4, 0, 0, 2, 0, 3, 0] # 79.61
