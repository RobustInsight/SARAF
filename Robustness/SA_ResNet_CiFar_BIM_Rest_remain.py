...
result_file = 'results_ResNet_Cifar10_BIM_total.csv'
...
if __name__ == '__main__':
    ...
    desired_record_seen = False
    for ith, state_ in enumerate(chromosomes):
        print('i-th chromosome :', ith)
        print (state_)
        if state_ == '0.25,4,0.25,2,1,3,4,6,0,5,0,5':
            print('yes')
            desired_record_seen = True
        else:
            if not desired_record_seen:
                continue
        ...
        for eps_ in eps_s:
            energy(model, state_, data_loaders_, 'BIM', eps_/255, test_acc_)
