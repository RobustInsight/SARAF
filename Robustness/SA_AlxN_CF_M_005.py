...
class robust_simannealer(Annealer):
    ...
    def energy(self):
        ...
        model = AlexNetM(num_classes=10, Activation_Functions=Activation_Functions, states_=self.state).to(device)

        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=0.001)
        fname = '../save/alx_cf10' + '_'.join(map(str, self.state)) + '.pth'
        ...
        write_data_in_file('results_AlexNet_Cifar_FGSM_base.csv', (self.state, self.atk, self.eps, rob_acc, test_acc))
